class GolfPlayRecord < ApplicationRecord
  ...
  def update_bestscore(user)
    ...
    if total_scores.any? { |score| score < best_score }
  ...
  def calculate_fairway_keep_percentage
    holes_with_tee_shot = self.scores.select { |score| score.tee_shot != "no_tee_shot" }
  ...
  def calculate_par_on_percentage
    on_green = self.scores.map { |score| score.content - score.putt }
    par_on_count = 0
  ...
  def calculate_within_200_to_150_par_on_percentage
    within_200_to_150_scores = self.scores.select { |score| score.second_shot_distance == "shot_within_two_hundredone_from_hundred_fifty" }
  ...
  def calculate_within_150_to_100_par_on_percentage
    within_150_to_100_scores = self.scores.select { |score| score.second_shot_distance == "shot_within_hundred_fifty_from_hundred" }
  ...
  def calculate_within_100_two_pin_percentage
    within_100_scores = self.scores.select { |score| score.approach_shot != "no_approach_shot" }
  ...
  def calculate_approach_success_rate
    holes_with_approach = self.scores.select { |score| score.approach != "no_approach" }
  ...
  def calculate_bunker_save_rate
    holes_with_bunker = self.scores.select { |score| score.bunker_save != "no_bunker" }
  ...
  def calculate_three_putt_percentage
    three_putts = self.scores.select { |score| score.putt >= 3 }
    return 0.0 if self.scores.empty?  # ゼロ除算を回避
    (three_putts.length.to_f / self.scores.length * 100).round(2)
  end

  def calculate_long_putt_success_rate
    holes_with_long_putt = self.scores.select { |score| score.long_putt != "no_long_putt" }
  ...
  def calculate_short_putt_success_rate
    holes_with_short_putt = self.scores.select { |score| score.short_putt != "no_short_putt" }
